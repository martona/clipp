...
constexpr unsigned char kMagic[4] = { 'C', 'L', 'P', 'R' };
constexpr uint8_t kVersion = 1;
constexpr size_t kHeaderSize = sizeof(kMagic) + 1;  // magic + version = the AEAD's AD
...
// The store cap is 1024 records; this only stops a forged count from driving
// the (authenticated, so practically unreachable) decode loop to absurdity.
constexpr uint32_t kMaxRecords = 65535;
...
std::vector<unsigned char> SealSnapshot(const std::vector<RegisterRecord>& records,
    ...
    // Count is patched in after the skip-on-encode-failure loop.
    AppendU32(plain, 0);
    uint32_t sealedCount = 0;
    for (const auto& record : records) {
        const std::vector<unsigned char> body = RegisterWire::EncodeRecord(record, 0);
        if (body.empty()) {
            continue;  // over-cap or otherwise unencodable — skip, never abort
        }
        AppendU32(plain, static_cast<uint32_t>(body.size()));
    ...
    sodium_memzero(plain.data(), plain.size());  // register values may be sensitive
...
bool TryOpenSnapshot(const std::vector<unsigned char>& blob, const SealKey& key,
    ...
    if (std::memcmp(blob.data(), kMagic, sizeof(kMagic)) != 0 || blob[4] != kVersion) {
        return false;
    }
    ...
    if (wellFormed) {
        ...
        for (uint32_t i = 0; i < count; ++i) {
            ...
            std::vector<unsigned char> body(plain.begin() + offset,
                                            plain.begin() + offset + len);
            offset += len;
...
bool SaveSnapshotFile(const std::string& utf8Path,
                      ...
                      const SealKey& key) {
    const std::vector<unsigned char> blob = SealSnapshot(records, key);
    if (blob.empty()) {
        return false;
    }
...
LoadResult LoadSnapshotFile(const std::string& utf8Path, const SealKey& key,
    ...
    if (!TryOpenSnapshot(blob, key, outRecords)) {
        quarantine();
        return LoadResult::Corrupt;
    }
